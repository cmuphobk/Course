...
class ViewController: UIViewController {
    ...
    var emojiArray: [Emoji] = [.smile, .cry, .scream]
    var emoji = [Int: Emoji]()
    
    @IBOutlet weak var countLabel: UILabel!
    
    lazy var game = Concentration(numberOfPairsOfCards:
        (self.cardButtons.count + 1) / 2)
    ...
    func updateViewModel() {
        for index in self.cardButtons.indices {
            ...
            if card.isFaceUp {
                ...
                button.backgroundColor = card.isMatched ? #colorLiteral(red: 1, green: 1, blue: 1, alpha: 0) : #colorLiteral(red: 1, green: 0.6910475492, blue: 0, alpha: 1)
            }
        }
    }
